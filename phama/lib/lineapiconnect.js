...
function createCarouselMainMenuTemplate(rows, shopid){
	...
	const imgLink = "https://www.myshopman.com/phama/img/upload/" + shopid + "/group/";
	const cob = [];
...
function createBubbleMenuTemplate(rows, shopid){
	return new Promise(function(resolve, reject) {
		const imgLink = "https://www.myshopman.com/phama/img/upload/" + shopid + "/item/";
		const cob = [];
		...
		rows.forEach((item)=>{
			var ob = {
				...
				imageBackgroundColor: "#000000",
				title: item.name,
				text: "ราคาขายปลีก " + item.unit + " ล่ะ " + item.price + " บาท\nราคาสมาชิก "  + item.exprice + " บาท",
				defaultAction: {
					type: "uri",
					label: "View Detail",
					uri: imgLink + item.picture
