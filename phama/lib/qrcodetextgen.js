...
module.exports.generateQRCodeText = function (qrtype, accountNo, totalCharge){
	...
	if (checksumcrc16ccit.length == 0){
		checksumcrc16ccit = '0000' + checksumcrc16ccit;
	} else if (checksumcrc16ccit.length == 1){
		checksumcrc16ccit = '000' + checksumcrc16ccit;
	} else if (checksumcrc16ccit.length == 2){
		checksumcrc16ccit = '00' + checksumcrc16ccit;
	} else if (checksumcrc16ccit.length == 3){
		checksumcrc16ccit = '0' + checksumcrc16ccit;
	}
...
/*
ใช้ CRC-16 และ ค่าคงที่ polynomial 0x1021 (XMODEM) พร้อมกับค่าเริ่มต้น 0xFFFF (อันนี้ต้องระวังเพราะไลบรารีส่วนมากมักใส่ค่าเริ่มต้นเป็น 0x0000) ตัวไลบรารีสามารถใช้ pycrc16 ได้โดยตรง
ชื่อ algorithm = CRC-CCITT (0xFFFF)
